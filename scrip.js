...
// ===================== Tema claro/escuro (paleta HBR Aviação) =====================
const THEME_STORAGE_KEY = "hbrTheme";

function applyTheme(theme) {
  const resolved = theme === "light" ? "light" : "dark";
  document.documentElement.setAttribute("data-theme", resolved);
  const toggleInput = document.getElementById("themeToggleInput");
  if (toggleInput) toggleInput.checked = resolved === "light";
}
...
function initTheme() {
  ...
  try {
    ...
    // localStorage indisponível (modo privado, por exemplo) — segue com o tema padrão.
  }
...
document.getElementById("themeToggleInput")?.addEventListener("change", (event) => {
  ...
  try {
    ...
    // Sem persistência disponível — o tema ainda se aplica nesta sessão.
  }
});
